...
import { validateTriageCitations } from "../../src/lib/citationVerifier.js";
...
請輸出標準 JSON 格式（勿包含 markdown 標籤或額外文字）：
{
  ...
  try {
    ...
    // If LLM produced the result (not rule-engine), downgrade confidence to medium
    if (!finalPayload.confidenceLevel) {
      finalPayload.confidenceLevel = "medium";
    }
    ...
    // Validate citations against verified statute database
    const citationValidation = validateTriageCitations(finalPayload.legalBasis || []);
    finalPayload.citationValidation = citationValidation;
    ...
    fallbackObj.confidenceLevel = "low";
    fallbackObj.error = "系統產生了預期外的錯誤，已自動回退到規則引擎層。";
    const citationValidation = validateTriageCitations(fallbackObj.legalBasis || []);
    fallbackObj.citationValidation = citationValidation;
