...
(async () => {
  ...
  const result = markLowConfidenceRegions([
    { text: '案號：112年度台上字第1號', confidence: 0.98 },
    { text: '姓名王小明', confidence: 0.42 },
    { text: '住址臺北市', confidence: 0.91 }
  ], 0.8);
  assert.equal(result.text, '案號：112年度台上字第1號[不確定]姓名王小明住址臺北市');
  assert.deepEqual(result.lowConfidenceRegions, [{ text: '姓名王小明', confidence: 0.42 }]);
  assert.equal(result.needsManualReview, true);
  assert.deepEqual(markLowConfidenceRegions([{ text: '清楚文字', confidence: 0.9 }]), {
