  ...
  const renderContent = () => {
    switch (activeTool) {
      case 'unified':
        return <UnifiedEntry />;
      case 'guide':
        return <LitigationWorkspace initialTab={initialData?.initialTab || 'guide'} initialToolId={initialData?.preselectedToolId} autoGenerate={initialData?.autoGenerate} />;
      case 'litigation':
        return <LitigationWorkspace initialTab={initialData?.initialTab === 'guide' ? 'toolbox' : initialData?.initialTab || 'toolbox'} initialToolId={initialData?.preselectedToolId} autoGenerate={initialData?.autoGenerate} initialFacts={initialData?.facts} />;
      case 'processGuide':
        return <LegalProcessGuide onNavigateToTool={handleSelectTool} />;
      case 'sdlc':
        return <LegalSdlcWorkbench />;
      case 'legalToolbox':
        return <LitigationWorkspace initialTab={initialData?.initialTab || 'toolbox'} initialToolId={initialData?.preselectedToolId} autoGenerate={initialData?.autoGenerate} initialFacts={initialData?.facts} />;
      case 'appeal':
      case 'smartAppeal':
      case 'appealDeadline':
        return (
          <LitigationWorkspace
            ...
            appealOnly
            autoGenerate={initialData?.autoGenerate}
          />
        );
      case 'agent-chat':
        return <AgentChat />;
