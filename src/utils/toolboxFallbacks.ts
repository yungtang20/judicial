 ...
 * 已有完整確定性模板的書狀類別。
 *
 * 這些類別不需要 AI 起草：模板本身就是確定性產物，沒有幻覺風險，
 * 也不會因為模型偶爾多寫一句法條而被引用查核擋下。
 * 實測先前全部走 AI 管線，導致 28 項工具中有 19 項產製失敗，
 * 同一份輸入的產出也會因模型隨機性而不同。
 */
export const DETERMINISTIC_TOOLBOX_CATEGORIES: ReadonlySet<string> = new Set([
  'ASSISTANCE_PETITION',
...
/** 該類別是否已有確定性模板可用。 */
export function hasDeterministicToolboxTemplate(category: string): boolean {
  return DETERMINISTIC_TOOLBOX_CATEGORIES.has(category);
}
