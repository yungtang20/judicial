...
const baseState: LegalWorkflowState = {
  ...
  syllogism: { majorPremise: '法律規則', minorPremise: '案件事實', subsumption: '要件比對', conclusion: '得請求損害賠償。', fullAnalysis: '完整分析內容', factMappings: [{ fact: '對方收款後未履行約定', statutes: [{ citation: '民法第184條', name: '侵權行為損害賠償', relation: '需確認故意、損害及因果關係' }], evidence: ['匯款紀錄', '雙方對話'] }] },
  verification: {
...
describe('UnifiedResult', () => {
  it('shows a verified result first, deduplicates evidence, and keeps reasoning collapsed', () => {
    ...
    expect(countMatchingPrecedents(baseState, '民法第184條（侵權行為）')).toBe(1);
    expect(screen.getByText('案件事實與法條、證據對照')).toBeInTheDocument();
    expect(screen.getByText('對方收款後未履行約定')).toBeInTheDocument();
    expect(screen.getByText('民法第184條｜侵權行為損害賠償')).toBeInTheDocument();
    expect(screen.getByText(/匯款紀錄/)).toBeInTheDocument();
    expect(screen.getByText('相關判例')).toBeInTheDocument();
    const sectionOrder = ['案件事實與法條、證據對照', '相關判例']
