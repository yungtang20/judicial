...
describe('legal toolbox classification', () => {
  it('offers one consistent set of life-situation categories', () => {
    ...
    for (const label of ['家庭與繼承', '借款與欠款', '住宅租賃']) {
      expect(screen.getByRole('button', { name: new RegExp(label) })).toBeInTheDocument();
    }
    fireEvent.click(screen.getByRole('button', { name: /借款與欠款/ }));
    expect(onSelectGroup).toHaveBeenCalledWith('DEBT_EXECUTION');
  });

  it('shows only tools that also exist in the reference toolbox', () => {
    expect(TOOLBOX_TOOLS.map(tool => tool.id)).toEqual([
    ...
    expect(TOOLBOX_GROUPS.filter(group => group.id !== 'ALL').every(group => TOOLBOX_TOOLS.some(tool => tool.categoryGroup === group.id))).toBe(true);
