  ...
  return (
    <>
          ...
          {(currentStage === 'PHASE_2' || (triageResult && triageResult.section1EvidenceRiskAssessment)) && (
            <div className="bg-[var(--color-surface-overlay)] border border-[var(--color-border-subtle)] rounded-xl p-6 shadow-sm space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-[var(--color-border-subtle)] gap-2">
                <div className="space-y-0.5">
                  <div className="flex items-center gap-2">
                    <span className="w-6 h-6 rounded-full bg-blue-100 text-[var(--color-status-info)] flex items-center justify-center text-xs font-bold">3</span>
                ...
                <div className="flex items-center gap-2 pl-8 sm:pl-0">
                  <button
                    ...
                    className="px-3 py-1.5 text-xs font-semibold bg-[var(--color-surface-overlay)] hover:bg-[var(--color-border-strong)] text-[var(--color-text-secondary)] rounded-lg transition-colors flex items-center gap-1"
                  >
              ...
              <div className="space-y-4">
                {/* Block 1 */}
                <div className="bg-[var(--color-surface-raised)] border border-[var(--color-border-subtle)] rounded-xl p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    <h3 className="font-bold text-[var(--color-text-primary)] text-xs flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      className="text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)] text-xs flex items-center gap-1"
                    >
                      {copiedSection === 'sec1' ? <Check className="w-3 h-3 text-emerald-600" /> : <Copy className="w-3 h-3" />} 複製
                    </button>
                  </div>
                  <p className="text-xs text-[var(--color-text-secondary)] whitespace-pre-wrap leading-relaxed font-sans">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-[var(--color-text-muted)]">標準事實調查問卷模板</span>
                </div>

                <div className="border border-[var(--color-border-subtle)] rounded-xl overflow-hidden shadow-2xs">
                  <table className="w-full text-left text-xs border-collapse">
                    <thead className="bg-[var(--color-surface-overlay)] text-[var(--color-text-secondary)] font-semibold border-b border-[var(--color-border-subtle)]">
                      <tr>
                    ...
                    <tbody className="divide-y divide-slate-200 bg-[var(--color-surface-overlay)]">
                      {(triageResult?.section3Questionnaire || []).map((q) => (
                        <tr key={q.qId} className="hover:bg-[var(--color-surface-raised)]/80 transition-colors">
                          <td className="p-3 font-mono font-bold text-[var(--color-text-muted)] text-center">{q.qId}</td>
                          <td className="p-3 font-bold text-[var(--color-text-primary)]">{q.title}</td>
                          <td className="p-3 space-y-1">
                            <div className="text-[var(--color-text-primary)] font-medium">{q.question}</div>
                            <div className="text-[11px] text-[var(--color-text-muted)]"><strong>引導指引：</strong>{q.guideNote}</div>
                            <div className="text-[11px] text-emerald-700"><strong>待證標的：</strong>{q.targetFact}</div>
                          </td>
                          <td className="p-3 text-[var(--color-text-secondary)] text-[11px]">
                            <span className="inline-block bg-[var(--color-surface-overlay)] border border-[var(--color-border-subtle)] px-2 py-1 rounded text-[var(--color-text-secondary)] font-medium">
              ...
              <div className="mt-6 pt-6 border-t border-[var(--color-border-subtle)] space-y-4 bg-[var(--color-status-warning-bg)] p-5 rounded-2xl border border-[var(--color-status-warning)]/80">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-[var(--color-status-warning)] text-sm flex items-center gap-1.5">
                  ...
                  <div 
                    ...
                    className={`p-4 rounded-xl border-2 transition-all cursor-pointer ${
                      gPointDecision === 'COOPERATE'
                        ...
                        : 'bg-[var(--color-surface-overlay)] border-[var(--color-border-subtle)] hover:border-emerald-300'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-bold text-[var(--color-status-success)] text-xs flex items-center gap-1.5">
                        <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                        【態度一】回歸理性配合提供事實
                      </span>
                      <span className="text-[10px] px-2 py-0.5 rounded bg-emerald-100 text-[var(--color-status-success)] font-bold">
                        走軌道一
                      </span>
                    </div>
                  ...
                  <div 
                    ...
                    className={`p-4 rounded-xl border-2 transition-all cursor-pointer ${
                      gPointDecision === 'INSIST_SUBMIT'
                        ...
                        : 'bg-[var(--color-surface-overlay)] border-[var(--color-border-subtle)] hover:border-rose-300'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-bold text-[var(--color-status-danger)] text-xs flex items-center gap-1.5">
                        <AlertOctagon className="w-4 h-4 text-rose-600" />
                        【態度二】堅持將全部意見送交法院
                      </span>
                      <span className="text-[10px] px-2 py-0.5 rounded bg-rose-100 text-[var(--color-status-danger)] font-bold">
                        啟動 Phase 3
                      </span>
                    </div>
                    <p className="text-xs text-[var(--color-text-secondary)] leading-relaxed mb-3">
