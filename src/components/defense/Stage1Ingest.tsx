...
export const Stage1Ingest: React.FC<Stage1IngestProps> = ({
  ...
  return (
    <div className="bg-[var(--color-surface-overlay)] border border-[var(--color-border-subtle)] rounded-xl p-6 space-y-4">
      <div className="flex items-center justify-between border-b border-[var(--color-border-subtle)] pb-3">
        <div className="space-y-0.5">
          <h2 className="text-base font-bold text-[var(--color-text-primary)] flex items-center gap-2">
            <span className="w-6 h-6 rounded-full bg-amber-100 text-[var(--color-status-warning)] flex items-center justify-center text-xs font-bold">1</span>
            當事人原始陳述、筆記與抱怨內容輸入
          </h2>
          <p className="text-xs text-[var(--color-text-muted)]">可直接貼上當事人之 LINE 對話、會議錄音摘要、手寫筆記或情緒性長文</p>
        </div>
        <span className="text-xs px-2 py-1 rounded bg-[var(--color-surface-overlay)] text-[var(--color-text-secondary)] font-mono">
      ...
      <div>
        <textarea
          ...
          className="w-full p-4 rounded-xl border border-[var(--color-border-subtle)] text-[var(--color-text-primary)] text-sm leading-relaxed focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none font-sans"
        />
      </div>
