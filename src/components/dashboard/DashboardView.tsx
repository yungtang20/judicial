...
export function DocumentBundleKit({ bundles, onSelectBundle, draftText, allowedCitations }: { bundles: string[]; onSelectBundle?: (bundleId: string) => void | Promise<void>; draftText?: string; allowedCitations?: string[] }) {
  ...
  return <section className="rounded-xl border border-slate-700 bg-slate-950/60 p-4" aria-label="文件組合包"><strong className="text-indigo-300">建議任務組合包</strong><div className="mt-2 flex flex-wrap gap-2">{bundles.length ? bundles.map(item => <button type="button" key={item} disabled={!onSelectBundle || busy !== null} onClick={() => void select(item)} className="rounded-lg border border-indigo-800 bg-indigo-950/60 px-2 py-1 text-xs hover:bg-indigo-900 disabled:cursor-wait disabled:opacity-60">{busy === item ? '產製中…' : item}</button>) : <span className="text-sm text-slate-400">尚無安全推薦。</span>}</div>{error && <p role="alert" className="mt-2 text-xs text-rose-300">{error}</p>}{draftText && <DraftRefiner draftText={draftText} allowedCitations={allowedCitations || []} />}</section>;
}
...
export default function DashboardView({ result, onSelectBundle }: { result: any; onSelectBundle?: (bundleId: string) => void | Promise<void> }) {
  const model = useMemo(() => buildDashboardModel(result), [result]);
  return <div className="space-y-3"><SafetyAndResourcePanel {...model.safety} /><PlainLanguageSummaryCard summary={model.summary} rights={model.rights} /><LegalConceptList concepts={Array.isArray(result?.legalBasis) ? result.legalBasis : []} status={result?.isComplete === false ? '待補充' : '待確認'} /><EvidenceChecklist evidence={model.evidence} /><DocumentBundleKit bundles={model.bundles} onSelectBundle={onSelectBundle} draftText={result?.pleadingDraft} allowedCitations={result?.allowedCitations || result?.antiGhostVerification?.verifiedCitations?.map((citation: any) => citation.citationText) || []} /><FilingGuideModal {...model.filing} /><AdvancedLegalReasoningPanel result={result} /></div>;
}
