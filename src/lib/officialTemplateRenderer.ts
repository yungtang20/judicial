...
/**
 * Build a mapping from ODT underline spans to manifest semantic keys.
 * Only the first N spans (where N = manifest fields with ODT positions) are mapped.
 * Remaining spans map to generic field_N keys for backward compatibility.
 */
function buildFieldMapping(template: OfficialTemplate): Array<{ odtStyle: string; key: string; label: string; required: boolean; placeholder?: string; type: string; options?: Array<{ label: string; value: string }> }> {
  ...
  const mapping: Array<{ odtStyle: string; key: string; label: string; required: boolean; placeholder?: string; type: string; options?: Array<{ label: string; value: string }> }> = [];

  for (let i = 0; i < UNDERLINE_STYLES.length; i++) {
    const style = UNDERLINE_STYLES[i];
    if (i < manifestFields.length) {
      const mf = manifestFields[i];
      mapping.push({
        odtStyle: style,
        key: mf.key,
        label: mf.label,
...
export function extractTemplateFields(template: OfficialTemplate): OfficialTemplateField[] {
  ...
  const absPath = path.resolve(process.cwd(), template.localFilePath);
  ...
  const mappedStyles = new Set(UNDERLINE_STYLES);
  const fields: OfficialTemplateField[] = [];
  const seenKeys = new Set<string>();

  for (const mf of (template.fields || [])) {
    if (seenKeys.has(mf.key)) continue;
    seenKeys.add(mf.key);

    // Check if this field has a corresponding ODT underline style
    const hasOdtSpan = mappedStyles.has(mf.key) || mappedStyles.has(`_${mf.key}`);
