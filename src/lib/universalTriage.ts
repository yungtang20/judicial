...
export function buildIntelligentRuleBasedTriage(query: string) {
    ...
    // ========== 硬分類前置攔截層：性暴力 / 妨害性自主 ==========
    // 偵測到性暴力關鍵字時，直接走規則引擎，不經過 LLM，
    // 確保刑法§221/§224/§229-1 一定出現在結果中，抑制民法§767 誤用。
    const SEXUAL_VIOLENCE_KEYWORDS = [
      "性侵", "強暴", "強制性交", "強姦", "非自願性行為", "妨害性自主",
      "強迫口交", "強迫性交", "按住頭", "壓住", "违背意願", "違反意願",
      "猥褻", "摸身體", "襲胸", "強吻", "逼迫脱衣"
    ];
    const isSexualViolence = SEXUAL_VIOLENCE_KEYWORDS.some(kw => q.includes(kw));
    ...
    if (isSexualViolence) {
      ...
      const isDomestic = q.includes("老公") || q.includes("老婆") || q.includes("丈夫") ||
                         q.includes("妻子") || q.includes("配偶") || q.includes("先生") ||
                         q.includes("另一半") || q.includes("伴侶") || q.includes("女朋友") ||
                         q.includes("男朋友");
      const hasImage = q.includes("拍攝") || q.includes("影像") || q.includes("影片") ||
                       q.includes("裸照") || q.includes("外流") || q.includes("偷拍");
      const isPenetration = q.includes("強迫口交") || q.includes("強迫性交") || q.includes("強制性交") ||
                            q.includes("異物插入") || q.includes("陰莖插入");
      ...
      if (isDomestic) {
        tools.push({
          ...
          reason: "行為人為配偶或同居伴侶，符合家庭暴力防治法，可一併聲請保護令禁止接近與騷擾。",
          urgency: "HIGH"
        });
      }
      ...
      // 配偶間性侵害特有法條
      const domesticStatutes = isDomestic ? [
        "家庭暴力防治法第63-1條（配偶間性侵害適用家暴法）",
        "民事訴訟法第167-1條（暫時狀態假處分/保護令）"
      ] : [];

      return {
        identifiedIssue: "妨害性自主被害案件" + (hasImage ? "暨未經同意散布性影像" : "") + (isDomestic ? "（配偶/伴侶家暴事件）" : ""),
        category: cat,
        caseType: "CRIMINAL_PUBLIC",
        confidenceLevel: "high" as const,
        detectedDomain: "CRIMINAL_AND_CIVIL",
        litigationNatureText: "⚡ 刑事非告訴乃論（公訴重罪，不因夫妻或伴侶關係而免訴追）",
        legalBasis: [
          "刑法第10條第5項（性交定義：以性器進入他人之性器、肛門或口腔，或以性器以外之他物進入他人之性器或肛門）",
          "刑法第221條（強制性交罪：對男女以強暴、脅迫、恐嚇、催眠術或其他違反其意願之方法而為性交者）",
          "刑法第224條（強制猥褻罪：對男女以強暴、脅迫、恐嚇、催眠術或其他違反其意願之方法而為猥褻行為者）",
          isPenetration ? "刑法第221條（強制性交既遂）" : "刑法第224條（強制猥褻）",
          isDomestic ? "刑法第229-1條（配偶間強制性交/猥褻，告訴乃論但得撤回告訴）" : "刑法第229-1條（告訴乃論規定）",
          isDomestic ? "家庭暴力防治法第63-1條（配偶間性侵害適用家暴法）" : "",
          hasImage ? "刑法第315條之1、第319條之3（散布性私密影像罪）" : "",
          "民法第184條（侵權行為損害賠償）",
          "民法第195條（非財產上損害賠償/精神慰撫金）"
        ].filter(Boolean),
        statuteAnalysis: "刑法第221條（強制性交罪，非告訴乃論公訴罪）、刑法第224條（強制猥褻罪）、" +
                         (isDomestic ? "刑法第229-1條（配偶間告訴乃論）、家庭暴力防治法§63-1、" : "") +
                         "民法第184條、第195條（精神慰撫金）",
        isPublicProsecution: !isDomestic, // 非配偶為公訴；配偶間為告訴乃論但仍構成犯罪
        statuteOfLimitations: isDomestic
          ? "【告訴乃論（配偶間性侵害）】應於知悉犯人之日起6個月內提出告訴（刑法§229-1）。但可同時聲請家暴保護令（無時效限制）。"
          : "【非告訴乃論（公訴重罪）】無6個月限制，檢警知悉即應主動追訴。黃金72小時內請至急診進行一站式性侵驗傷採證。",
        timeLimit: isDomestic
          ? "告訴乃論 6 個月（刑法§229-1）；保護令無時效限制"
          : "【非告訴乃論】公訴重罪無6個月限制",
        plainExplanation: "違反意願之性交或猥褻行為，「即使發生在配偶或伴侶之間」，仍然構成刑法上的犯罪。" +
          (isPenetration
            ? "被害人所述「強迫口交」之行為，依刑法第10條第5項之性交定義，以性器進入他人之口腔即屬性交行為，因此構成刑法第221條強制性交罪。"
            : "被害人所述之強制猥褻行為，構成刑法第224條強制猥褻罪。") +
          (isDomestic
            ? "因行為人為配偶，依刑法第229-1條規定，此罪為告訴乃論，但被害人仍可依法提出告訴，且可同時依家庭暴力防治法聲請保護令。"
            : "此為非告訴乃論公訴重罪，檢警知悉即應主動追訴。") +
          (hasImage ? "若有未經同意拍攝或散布性影像，另構成妨害秘密與性私密影像犯罪。" : "") +
          "「民法第767條（物上請求權）」係用於請求返還所有物，與性自主權侵害無關，不應適用於本案。",
        recommendedAction: isDomestic
          ? "1. 72小時內急診驗傷採證（勿洗澡更衣） 2. 撥打113專線 3. 向地檢署提出告訴 4. 聲請家暴保護令 5. 提起附帶民事訴訟求償精神慰撫金。"
          : "1. 72小時內急診驗傷採證（勿洗澡更衣） 2. 保全對話與影像證據 3. 撥打113或向地檢署提出告訴 4. 聲請保護令 5. 提起附帶民事訴訟求償精神慰撫金。",
        suggestedActions: [
    ...
    if (q.includes("性侵") || q.includes("強暴") || q.includes("非自願性行為") || q.includes("妨害性自主") || q.includes("強制性交") || q.includes("強姦")) {
      ...
      return {
        ...
        statuteAnalysis: "刑法第221條（強制性交罪）、刑法第224條（強制猥褻罪）、" +
                         (isDomestic ? "刑法第229-1條（配偶間告訴乃論）、家庭暴力防治法§63-1、" : "") +
                         "民法第184條、第195條（精神慰撫金）",
        isPublicProsecution: true,
        statuteOfLimitations: isDomestic
        ...
        recommendedToolId: cat,
        recommendedTools: tools,
        readyDocumentTitle: "妨害性自主告訴狀（待律師協助撰擬）",
        readyDocumentText: "本案涉及刑法第221條強制性交罪或第224條強制猥褻罪，建議委由律師協助撰擬正式刑事告訴狀。",
        pleadingDraft: "本案涉及刑法第221條強制性交罪或第224條強制猥褻罪，建議委由律師協助撰擬正式刑事告訴狀。",
        complianceChecklist: ["72小時內急診驗傷採證", "撥打113專線", "向地檢署提出告訴", isDomestic ? "聲請家暴保護令" : ""].filter(Boolean),
        antiGhostVerification: "性侵害案件文件須經律師審閱確認，禁止AI自行產出空白模板。"
      };
    }
