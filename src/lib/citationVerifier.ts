...
/**
 * Validate triage legalBasis array against VERIFIED_REAL_STATUTES.
 * Extracts statute article names from each legal basis string and checks them.
 * Returns valid and unrecognized lists so callers can surface confidence to judges.
 */
export function validateTriageCitations(legalBasis: string[]): {
  valid: string[];
  unrecognized: string[];
  totalChecked: number;
} {
  ...
  // Patterns to extract statute name from legal basis strings
  // e.g. "刑法第221條（強制性交罪）" → "刑法第221條"
  // e.g. "民法第184條（侵權行為損害賠償）" → "民法第184條"
  const extractPatterns = [
    // Standard: 法名第N條
    /(民法|民事訴訟法|刑法|刑事訴訟法|票據法|勞動基準法|強制執行法|家事事件法|家庭暴力防治法|非訟事件法)第([0-9０-９]+)(?:條之([0-9０-９]+))?條/,
    // Shortcut: 法名§N (e.g. "刑法§221")
    /(民法|民事訴訟法|刑法|刑事訴訟法|票據法|勞動基準法|強制執行法|家事事件法|家庭暴力防治法|非訟事件法)§([0-9０-９]+)(?:-([0-9０-９]+))?/,
    // With sub-article marker: 民法第1030條之1
    /(民法|民事訴訟法|刑法|刑事訴訟法|票據法|勞動基準法|強制執行法|家事事件法|家庭暴力防治法|非訟事件法)第([0-9０-９]+)條之([0-9０-９]+)/,
  ];
  ...
  for (const basis of legalBasis) {
    ...
    for (const pattern of extractPatterns) {
      ...
      if (m) {
        ...
        const keys = subArt
          ? [
              `${lawName}第${mainArt}條之${subArt}`,
              `${lawName}第${mainArt}之${subArt}條`,
              `${lawName}§${mainArt}-${subArt}`,
            ]
          : [
              `${lawName}第${mainArt}條`,
              `${lawName}§${mainArt}`,
        ...
        const found = keys.some(k => k in VERIFIED_REAL_STATUTES);
        if (found) {
          valid.push(basis);
          matched = true;
        }
  ...
  return {
    valid,
    unrecognized,
    totalChecked: legalBasis.length,
  };
}
